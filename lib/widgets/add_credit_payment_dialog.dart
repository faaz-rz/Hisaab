...
class AddCreditPaymentDialog extends ConsumerStatefulWidget {
  const AddCreditPaymentDialog({super.key});

  @override
  ConsumerState<AddCreditPaymentDialog> createState() =>
      _AddCreditPaymentDialogState();
}

class _AddCreditPaymentDialogState
    extends ConsumerState<AddCreditPaymentDialog> {
  ...
  // Unpaid bills for the selected agency
  ...
  void _savePayment() async {
    ...
    // Validate amount against selected bill
    if (_selectedBill != null && amount > _selectedBill!.remainingAmount) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Amount exceeds remaining balance of ₹${_selectedBill!.remainingAmount.toStringAsFixed(0)}'),
    ...
    final tx = TransactionModel(
      ...
      originalBillNo: _selectedBill?.billNo,
      paymentMethod: _paymentMethod,
      receiptNo:
          _paymentMethod == 'cash' && _receiptNoCtrl.text.trim().isNotEmpty
  ...
  Widget _buildUnpaidBillsList() {
    ...
    if (_agencyCodeCtrl.text.trim().isEmpty &&
        _agencyNameCtrl.text.trim().isEmpty) {
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Paying Bill: ${_selectedBill!.billNo ?? "N/A"} — Remaining: ₹${_selectedBill!.remainingAmount.toStringAsFixed(0)}',
                              style: GoogleFonts.inter(
                                  fontSize: 12,
                                  fontWeight: FontWeight.w500,
